...
template <class Functor>
__global__ void apply_f_frnn_gc_kernel(Functor f, int numP, glm::vec2 *pos, int *count_keys, int *cumulative_count_keys, float rad_2, GridCount2d_data *gcdata)
{
	const int i = threadIdx.x + (blockDim.x * blockIdx.x);
	if (i < numP)
	{
		...
		for (int ddx = -1; ddx <= 1; ddx++)
		{
			for (int ddy = -1; ddy <= 1; ddy++)
			{
				...
				if (hh.x > gcdata->num_cells.x || hh.x < 0)
					continue;
				if (hh.y > gcdata->num_cells.y || hh.y < 0)
					continue;
